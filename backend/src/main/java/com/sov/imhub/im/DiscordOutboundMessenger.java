...
/**
 * Discord 出站消息：使用 Discord Interactions 回复或 Webhook 发送消息。
 */
@Slf4j
@RequiredArgsConstructor
public final class DiscordOutboundMessenger implements OutboundMessenger {
    ...
    private void sendPlain(String text) {
        ...
        if (botToken == null || botToken.isBlank()) {
            log.warn("Discord bot token not configured for channelId={}", channel.getId());
            return;
        }
        String chatId = ctx.externalChatId();
        if (chatId == null || chatId.isBlank()) {
            log.warn("Discord channel_id empty for channelId={}", channel.getId());
            return;
        }
        try {
            ...
            log.warn("Discord sendPlain failed channelId={}: {}", channel.getId(), e.getMessage());
        }
    }
