...
import com.sov.imhub.service.telegram.TelegramChatAccessService;
import com.sov.imhub.service.telegram.TelegramCommandParser;
import com.sov.imhub.service.telegram.WebhookRateLimiter;
...
public class QueryOrchestrationService {
        ...
        try {
            ...
            if (!webhookRateLimiter.allowKey(rateLimitKey(ctx))) {
                ...
                commandLogService.log(
                        ...
                        null,
                        false,
                        CommandLogService.KIND_RATE_LIMIT,
                        logStarted,
                        null);
                return;
            }
            ...
            if (!isAllowlisted(ctx)) {
                ...
                commandLogService.log(
                        ...
                        null,
                        false,
                        CommandLogService.KIND_NOT_ALLOWED,
                        logStarted,
                        null);
                return;
            }
                ...
                if (isAllArgsBlank(parsed)) {
                    ...
                    commandLogService.log(
                        ...
                        ctx.externalChatId(),
                            ...
                            logQueryId,
                            false,
                            CommandLogService.KIND_MISSING_PARAM,
                            logStarted,
            ...
            commandLogService.log(
                    ...
                    logQueryId,
                    true,
                    CommandLogService.KIND_SUCCESS,
                    logStarted,
                    "rows=" + rows.size() + "; " + CommandLogService.paramNamesOnly(pnames));
        } catch (Exception e) {
            log.warn(
            ...
            if (!logCommand.isEmpty()) {
                commandLogService.log(
                        ...
                        logQueryId,
                        false,
                        CommandLogService.KIND_QUERY_FAILED,
    ...
    private boolean isAllowlisted(InboundCommandContext ctx) {
        if (ctx.channelId() == null) {
            // Fallback for legacy Telegram without channel
            return true;
        }
        return channelAllowlistService.isAllowed(ctx.channelId(), ctx.externalUserId());
    }
